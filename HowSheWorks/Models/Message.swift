...
class Message: Identifiable, ObservableObject {
    ...
    init(role: Role, content: String, isComplete: Bool = false, images: [URL] = [], videos: [URL] = []) {
        self.id = UUID()
        self.role = role
...
extension Message {
    ...
    static func user(_ content: String, images: [URL] = [], videos: [URL] = []) -> Message {
        Message(role: .user, content: content, isComplete: true, images: images, videos: videos)
    }
